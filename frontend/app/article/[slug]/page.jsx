import axios from "axios";
import Link from "next/link";

async function getArticle(slug) {
  const res = await axios.get(
    `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/article/articleslug/${slug}`,
  );
  return res.data.articles;
}

async function getLatestArticles() {
  const res = await axios.get(
    `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/article/fourarticles`,
  );
  return res.data.articles;
}

export default async function ArticleDetail({ params }) {
  const { slug } = await params;

  const article = await getArticle(slug);
  const latestArticles = await getLatestArticles();

  if (!article) {
    return <h1>Articles Not Found</h1>;
  }

  return (
    <>
      <div className="max-w-7xl mx-auto px-6 py-16 mt-15">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 mt-15">
          {/* LEFT SIDE ARTCILE */}
          <div className="lg:col-span-2">
            <div className="bg-white rounded-xl shadow p-6">
              <Link
                href="/insights/article"
                className="text-blue-900 text-lg flex items-center gap-2 mb-6 font-medium "
              >
                ← Back to Articles
              </Link>

              <img
                src={`${process.env.NEXT_PUBLIC_BACKEND_URL}${article.articleImg.url}`}
                alt={article.articleName}
                className="w-full h-[350px] object-contain mb-6 rounded border border-gray-200"
              />

              <h1 className="text-4xl font-bold text-blue-900 mb-4">
                {article.articleName}
              </h1>

              <div className="flex items-center text-sm text-gray-500 gap-4 mb-6">
                <span>
                  {" "}
                  📅 Published on{" "}
                  {new Date(article.articleDate)
                    .toLocaleDateString("en-GB", {
                      day: "2-digit",
                      month: "short",
                      year: "numeric",
                    })
                    .replace(/ /g, "-")}
                </span>{" "}
                <span>✍️ By Admin</span>
              </div>

              <div
                className="prose max-w-none text-gray-700"
                dangerouslySetInnerHTML={{ __html: article.articleDetail }}
              />
            </div>
          </div>

          {/* RIGHT SIDEBAR */}
          <div>
            <div className="bg-white rounded-xl shadow p-6 ">
              <h2 className="text-xl font-semibold mb-6 text-blue-900">
                Latest Posts
              </h2>

              <div className="space-y-5">
                {latestArticles.map((item) => (
                  <Link
                    key={item._id}
                    href={`/article/${item.slug}`}
                    className="flex items-start gap-4 hover:bg-gray-50 p-2 rounded-lg transition"
                  >
                    <img
                      src={`${process.env.NEXT_PUBLIC_BACKEND_URL}${item.articleImg.url}`}
                      alt={item.articleName}
                      className="w-17 h-19 object-cover rounded-md border shadow border-gray-200"
                    />

                    <div>
                      <p className="text-m font-semibold text-gray-800 leading-snug">
                        {item.articleName}
                      </p>

                      <p className="text-xs text-gray-500">
                        Published on {new Date(item.articleDate).toDateString()}
                      </p>
                    </div>
                  </Link>
                ))}
              </div>
            </div>
          </div>
        </div>
      </div>
    </>
  );
}
