...
const ListArticles = () => {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/article/delete/${id}`,
      );
  ...
  return (
    <div className="p-4 md:p-8 bg-gray-50 min-h-screen">
      ...
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        {/* Search input (left) */}
        <div className="relative w-full md:max-w-md flex-1 min-w-[200px]">
        ...
        <Link href="/subadmin/article">
          <button className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg transition shadow-sm whitespace-nowrap">
            + Add Article
          </button>
        </Link>
      </div>
  ...
  .replace(/ /g, "-")}</td>
                    ...
                    <td className="p-4 text-center">
                      <div className="flex justify-center gap-3">
                        <button
                          onClick={() =>
                            router.push(`/subadmin/article/${article._id}`)
                          }
