  ...
  const handleLogin = async (e) => {
    ...
    try {
      const res = await axios.post(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/subadmin/login`,
        { email, password },
      );
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-slate-100 p-4">
      <div className="w-full max-w-6xl flex flex-col md:flex-row bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100 transform transition-all duration-300 hover:shadow-2xl">
        ...
        <div className="hidden md:block md:w-1/2 relative overflow-hidden">
          {/* Image */}
          <img
            src="/login/subadmin.jpg"
            alt="Admin Illustration"
            className="h-full w-full object-cover transform hover:scale-105 transition-transform duration-700  rounded-2xl"
          />
          ...
          <div className="relative z-10">
            ...
            <div className="text-center mb-10">
              ...
              <p className="text-gray-500 text-lg">
                Enter your credentials to access the sub admin dashboard
              </p>
            </div>

            {error && (
              <div className="mb-6 p-4 bg-red-50 border border-green-200 text-green-700 rounded-lg flex items-start gap-3 transition-all duration-300 animate-fadeIn">
                <FiAlertCircle className="mt-0.5 flex-shrink-0 text-green-500" />
            ...
            <form onSubmit={handleLogin} className="space-y-6">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full py-4 bg-green-600 text-white rounded-lg text-base font-medium shadow-sm hover:shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-all duration-300 disabled:opacity-80 disabled:cursor-not-allowed flex items-center justify-center group"
              >
                {loading ? (
                  <>
                    <svg
            ...
            <div className="mt-10 pt-6 border-t border-gray-100 text-center text-sm text-gray-500">
              <p>
                © {new Date().getFullYear()} eRIC SOLUTIONS. All rights
                reserved.
              </p>
            </div>
          </div>
