  ...
  const handleLogin = async (e) => {
    ...
    try {
      const res = await axios.post(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/viewer/login`,
        { email, password },
      );
      ...
      <div className="w-full max-w-6xl flex flex-col md:flex-row bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100 transform transition-all duration-300 hover:shadow-2xl">
        ...
        <div className="w-full md:w-1/2 p-8 md:p-12 flex flex-col justify-center relative">
          ...
          <div className="relative z-10">
            ...
            {error && (
              <div className="mb-6 p-4 bg-red-50 border border-blue-200 text-blue-700 rounded-lg flex items-start gap-3 transition-all duration-300 animate-fadeIn">
            ...
            <form onSubmit={handleLogin} className="space-y-6">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full py-4 bg-blue-600 text-white rounded-lg text-base font-medium shadow-sm hover:shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-all duration-300 disabled:opacity-80 disabled:cursor-not-allowed flex items-center justify-center group"
              >
                {loading ? (
                  <>
                    <svg
